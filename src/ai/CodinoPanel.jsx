...
export default function CodinoPanel({ open, pinned, onTogglePin, context, onClose }) {
  ...
  const historyFor = (id) => {
    ...
    const prior = (t ? t.messages : []).map((m) => ({ role: m.role, content: m.content }));
    return [{ role: "system", content: `${ctx?.q ? EXPLAIN_SYSTEM : ASK_SYSTEM}\n\n${FORMAT_CONTRACT}` }, ...prior];
  };
